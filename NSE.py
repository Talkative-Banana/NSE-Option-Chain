...
# Reorder columns: Call | Strike | Put
df = df[["Call", "", "Put"]]
...
def highlight(df):
    ...
    for col in [("Call", "D%"), ("Put", "D%"), ("Call", "NetVol"), ("Put", "NetVol"),
                ("Call", "OI_Chg%"), ("Put", "OI_Chg%")]:
        styled = styled.highlight_max(subset=[col], color="#00ff00")
    ...
    styled = styled.apply(lambda r: ["background-color:#ff0000" if r[("", "Strike")] == atm else "" for _ in r], axis=1)
    # Highlight Strike column
    styled = styled.applymap(lambda v: "font-weight:bold; background-color:#ff0000", subset=[("", "Strike")])
    return styled
